<?php
	...
	while($counter<$size){
		if(isset($_POST[$active[$counter]])){
			...
			$reg = true;
			$num = $active[$counter];
			$query =  $handler->query("SELECT available FROM regactivity WHERE acid ='$num'");
			$query2 = $handler->query("SELECT name FROM regactivity WHERE acid='$num'");
			$query3 = $handler->query("SELECT Price FROM regactivity WHERE acid='$num'");
			...
			//get name
			$memname = $handler->query("SELECT name FROM signedmembers WHERE memberId='$memid'");
			$memname1 = $memname->fetch();
			$memname2= $memname1[0];
			
			//get email
			$mememail = $handler->query("SELECT email FROM signedmembers WHERE memberId='$memid'");
			$mememail1 = $mememail->fetch();
			$mememail2= $mememail1[0];
			
			//get mobile
			$memmobile = $handler->query("SELECT mobilenumber FROM signedmembers WHERE memberId='$memid'");
			$memmobile1 = $memmobile ->fetch();
			$memmobile2= $memmobile1[0];		
			...
			$myplace = $myplace-1;
			$sql = "UPDATE `regactivity` SET `available`=:myplace , `resname`=:memname2 , `memid`=:memid WHERE `acid`=:num";
			...
			// execute the query
			$pdoExec=$pdoResult->execute(array(":myplace"=>$myplace,":num"=>$num,":memname2"=>$memname2,":memid"=>$memid));
